      ...
      <div className="container-px">
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-4">
          {plans.map((plan, i) => {
            ...
            return (
              <Reveal key={meta.name} delay={i * 0.07} className="h-full">
                <div
                  className={cn(
                ...
                >
                  ...
                  <h3
                    className={cn(
                      "text-lg font-bold",
                      meta.highlighted ? "text-white" : "text-ink-900",
                    )}
                  >
                    {meta.name}
                  </h3>
                  <p
                    className={cn(
                      "mt-1 text-sm",
                      meta.highlighted ? "text-white/60" : "text-ink-500",
                    )}
                  >
                    {plan.tagline}
                  </p>
                  ...
                  <div className="mt-5 flex items-baseline gap-1.5">
                    ...
                    <span
                      className={cn(
                        "text-sm",
                        meta.highlighted ? "text-white/50" : "text-ink-400",
                      )}
                    >
                      {plan.period}
                    </span>
                  ...
                  <Link
                    ...
                    className={cn(
                      "mt-6 w-full",
                      meta.highlighted ? "btn bg-white text-ink-900 hover:bg-ink-100" : "btn-secondary",
                    )}
                  >
                    {plan.cta}
                  </Link>
                      ...
                      <li key={feat} className="flex items-start gap-2.5 text-sm">
                        <Check
                          className={cn(
                            ...
                            meta.highlighted ? "text-brand-300" : "text-brand-600",
                          )}
                        />
                        <span
                          className={meta.highlighted ? "text-white/80" : "text-ink-600"}
                        >
                          {feat}
                        </span>
                      </li>
